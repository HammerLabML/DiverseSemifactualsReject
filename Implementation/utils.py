...
def evaluate_closeness(xcf, x_orig):
    return np.linalg.norm(xcf - x_orig, 2)
...
def evaluate_sparsity_ex(x):
    return np.sum(np.abs(x[i]) > non_zero_threshold_sparsity for i in range(x.shape[0])) / x.shape[0]    # Count non-zero features (smaller values are better!)
...
def evaluate_perturbed_features_recovery_ex(x, perturbed_features_idx):
    ...
    tp_ = np.sum([idx in perturbed_features_idx for idx in indices])
    fp_ = np.sum([idx not in perturbed_features_idx for idx in indices])
    tn_ = np.sum([idx not in perturbed_features_idx for idx in filter(lambda i: i not in indices, range(x.shape[0]))])
    fn_ = np.sum([idx in perturbed_features_idx for idx in filter(lambda i: i not in indices, range(x.shape[0]))])
